from environment.mujoco.hopper import Hopper
...
def collect_ranking_data_llm(env:Hopper, filepath, llm_filepath, sample_size,start_id=0):
...
def generate_training_data(env:Hopper, num_samples = 1):
    training_data = {"input": [], "target": [], "llm_target": []}
...
def create_training_data(args):
    env = Hopper(exclude_current_positions_from_observation=False)
