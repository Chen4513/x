...
class ResultReaderFromFile:
    ...
    def read_results(self, smooth_weight:float):
        ...
        for index, row in data.iterrows():
            ...
            #self.episode_length_mean.append(row['episode_len_mean'])
    ...
    def plot_results(self):
        self.read_results()
        plt.figure(1)
        plt.plot(self.time_steps, self.episode_reward_mean)
        plt.title("episode reward mean")
        plt.xlabel("training time steps")
        plt.show()
        plt.figure(2)
        plt.plot(self.time_steps, self.episode_length_mean)
        plt.xlabel("training time steps")
