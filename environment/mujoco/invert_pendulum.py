...
class InvertedPendulum(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, config, max_steps=1000, **kwargs):
    ...
    def step(self, a):
        reward = 1
        self.do_simulation(a, self.frame_skip) # default frame skip is 2
        ob = self._get_obs()
    ...
    def viewer_setup(self):
        ...
        v = self.viewer
        v.cam.trackbodyid = 0
        v.cam.distance = self.model.stat.extent+2
        self.viewer.cam.lookat = [0, 0, 0]

    
    def randomize_state(self):
        qpos_range = np.array([1,0.2])
        qvel_range = np.array([10,10])
        qpos = self.init_qpos + self.np_random.uniform(
            size=self.model.nq, low=-qpos_range, high=qpos_range
        )
        qvel = self.init_qvel + self.np_random.uniform(
            size=self.model.nv, low=-qvel_range, high=qvel_range
        )
        self.set_state(qpos, qvel)
        return self._get_obs()