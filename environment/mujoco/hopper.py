...
class Hopper(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def randomize_state(self):
        q_noise_high = np.array([0.05, 0.05, 0.1, 0.1, 0.1, 0.1, 0.05, 0.05, 0.05, 0.2, 0.2, 0.3, 0.3, 0.3, 0.2, 0.2, 0.1])
        q_noise_low = np.array([-0.05, -0.05, -0.1, -0.1, -0.1, -0.1, -0.05, -0.05, -0.05, -0.2, -0.2, -0.3, -0.3, -0.3, -0.2, -0.2, -0.1])

        qpos = self.init_qpos + self.np_random.uniform(
            low=q_noise_low, high=q_noise_high, size=self.model.nq
        )
        qvel = self.init_qvel + self.np_random.uniform(
            low=-10, high=10, size=self.model.nv
        )
